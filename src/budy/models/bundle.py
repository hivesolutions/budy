...
class Bundle(base.BudyBase):
    ...
    country = appier.field(index="hashed")

    quantity = appier.field(
        type=commons.Decimal, index=True, initial=commons.Decimal(0.0), safe=True
    )
    ...
    referrals = appier.field(
        type=appier.references("Referral", name="name"), eager=True
    )
    ...
    def add_product_s(
        ...
        if not product:
            raise appier.OperationalError(message="No product defined")
    ...
    def refresh_s(self, currency=None, country=None, force=False):
        ...
        for line in lines:
            is_dirty = line.is_dirty(currency=currency, country=country)
            if not is_dirty:
                continue
    ...
    def calculate(self):
        ...
        self.undiscounted_sub_total = sum(
            line.total for line in lines if not line.discounted
        )
        self.discountable_sub_total = sum(
            line.total
            for line in lines
            if line.is_discountable(strict=not self.discountable_full)
        )
    ...
    def build_discount(self):
        ...
        self.discount_dynamic = self.__class__.eval_discount(
            self.discountable, self.taxes, self.quantity, self
        )
        if join_discount:
    ...
    def build_taxes(self):
        ...
        taxes_dynamic = self.__class__.eval_taxes(
            self.sub_total, self.taxes, self.quantity, self
        )
    ...
    def is_dirty(self, currency=None, country=None):
        ...
        lines = self.lines if hasattr(self, "lines") else []
        for line in lines:
            dirty |= line.is_dirty(currency=currency, country=country)
        return dirty
    ...
    def discountable(self):
        return (
            self.sub_total + self.shipping_cost
            if self.discountable_full
            else self.discountable_sub_total
        )
