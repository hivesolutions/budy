...
class BudyAccount(appier_extras.admin.Account):
    PREFIXES = appier_extras.admin.Account.PREFIXES + ["budy."]

    GENDER_S = dict(Male="Male", Female="Female")

    first_name = appier.field(index=True)

    last_name = appier.field(index=True)

    gender = appier.field(meta="enum", enum=GENDER_S)

    birth_date = appier.field(type=int, index=True, meta="date")
    ...
    store = appier.field(type=appier.reference("Store", name="id"), eager=True)

    addresses = appier.field(type=appier.references("Address", name="id"))
    ...
    def _build(cls, model, map):
        ...
        if id:
            model["bag_key"] = cls._get_bag_key(id)
        if full_name:
    ...
    @appier.operation(
        ...
        parameters=(("CSV File", "file", "file"), ("Strict", "strict", bool, False)),
    )
    ...
    def simple_csv_url(cls, absolute=False):
        return appier.get_app().url_for("account_api.simple_csv", absolute=absolute)
    ...
    def recover_s(self, send_email=False):
        result = appier_extras.admin.Account.recover_s(self, send_email=send_email)
    ...
    def get_store_orders(self, status="waiting_payment"):
        if not self.store:
            raise appier.OperationalError(
                message="No store associated with user", code=403
            )
        kwargs = dict()
        if self.store.is_restricted:
            kwargs["account"] = self.id
        orders = order.Order.find(
            status=status, store=self.store.id, sort=[("id", -1)], **kwargs
        )
        return orders

    @appier.operation(
        name="Set Store",
        parameters=(("Store", "store", appier.reference("Store", name="id")),),
    )
    ...
    @appier.operation(
        name="Set Metadata", parameters=(("Metadata", "meta", "longmap"),)
    )
    ...
    def notify(self, name=None, *args, **kwargs):
        ...
        appier_extras.admin.Event.notify_g(
            ...
            arguments=dict(
                params=dict(
                    payload=account, account=account, receiver=receiver, extra=kwargs
                )
            ),
        )
